    ...
    return (
        ...
        >
            <div className="flex items-center justify-between gap-3 mb-2.5 pb-2 border-b border-slate-800">
                ...
                <span
                    className={`px-2 py-0.5 rounded text-[10px] font-bold ${isPareto
 ...
 * PlotArtifact
 * -------------------------------------------------------------
 * Renders a backend-generated PNG (XGBoost importance / SHAP summary) with
 * a graceful, honest fallback chain instead of a silently-broken <img> or
 * an unrelated stock placeholder image:
 *   1. Try each URL in `sources` in order (e.g. local /public path, then a
 *      backend API path) using onError to advance through the list.
 *   2. If every source fails, show a clear "not generated yet" message with
 *      a Retry button, and disable click-to-zoom so the lightbox never
 *      opens on a broken image.
 *
 * NOTE: `sources` currently assumes the PNG may be bundled as a static file
 * under /public (e.g. "/xgboost_importance.png") OR served by the backend
 * at `${API_BASE}/api/plots/...`. If your backend serves these plots at a
 * different route, update the `sources` arrays where PlotArtifact is used
 * below so the first successful match wins.
 */
function PlotArtifact({
    ...
    imgClass = "max-h-[320px] w-full object-contain rounded",
    onOpen,
}: {
    sources: string[];
    alt: string;
    ...
    return (
        <div
            onClick={() => !failed && onOpen(currentSrc)}
            className={`rounded-lg overflow-hidden border border-slate-800/80 bg-black/60 p-2 flex items-center justify-center ${heightClass} ${failed ? "cursor-default" : "cursor-pointer hover:border-slate-700 transition-colors"
                }`}
        >
            {failed ? (
                <div className="flex flex-col items-center justify-center gap-2 text-center py-6 px-4">
                    <ImageIcon className="h-7 w-7 text-slate-600" />
                    <p className="text-[11px] text-slate-500 max-w-[240px]">
                        {label} hasn&apos;t been generated for this dataset yet. Upload telemetry to render it, or
                        check that your backend is serving this plot.
                    </p>
                    <button
                        onClick={retry}
...
export default function ObservabilityDashboard() {
    ...
    // Fixed preset widths instead of free-drag resizing (see note above the
    // removed drag-resize effect below) — click-cycled, not dragged, so
    // charts inside the panel never re-flow mid-gesture.
    const RIGHT_PANEL_WIDTHS = [380, 460, 560] as const;
    const [rightPanelWidthIdx, setRightPanelWidthIdx] = useState(1);
    const rightPanelWidth = RIGHT_PANEL_WIDTHS[rightPanelWidthIdx];
    const [overviewViewMode, setOverviewViewMode] = useState<"all" | "interactive" | "xgboost_png" | "shap_png">("all");
    const [lightboxImage, setLightboxImageRaw] = useState<{ src: string; title: string; desc: string } | null>(null);
    const [lightboxLoadFailed, setLightboxLoadFailed] = useState(false);
    const setLightboxImage = (img: { src: string; title: string; desc: string } | null) => {
        setLightboxLoadFailed(false);
        setLightboxImageRaw(img);
    };
    ...
    // NOTE: the previous drag-to-resize handle for the right panel has been
    // removed — its 6px hit-target sat exactly on the panel boundary, was
    // hard to grab reliably, and continuous width changes while dragging
    // forced every chart inside the panel (ResponsiveContainer) to re-measure
    // and re-render on each mousemove, causing visible jank. It's replaced by
    // a simple click-to-cycle width button (Narrow / Normal / Wide) below,
    // plus the new left-panel collapse toggle for reclaiming horizontal space.

    // Trace Fetcher — reports the run's real registration/trace state honestly.
    // Rather than fabricating fake log lines when the backend has no events for
    // a run (which hid the real "not registered" condition from the user), this
    // surfaces a clear, actionable message so the user knows to upload/ingest
    ...
    const inspectRun = useCallback(async (runId: string) => {
        ...
        try {
            ...
            const events: TraceEvent[] = res.data.events || res.data.trace || res.data.logs || [];
            ...
            if (!events || events.length === 0) {
                setTraceError(
                    `No trace log is registered for run ${runId} yet. Upload the matching .json/.jsonl/.txt log via "Upload" to view execution details.`
                );
            }
        } catch (err: unknown) {
            console.error("Failed to fetch trace:", err);
            const anyErr = err as { response?: { status?: number; data?: { detail?: string; error?: string; message?: string } } };
            const backendMsg =
                anyErr.response?.data?.detail || anyErr.response?.data?.error || anyErr.response?.data?.message;
            if (anyErr.response?.status === 404) {
                setTraceError(backendMsg || `Run ${runId} is not registered on the backend. It may not have been ingested yet.`);
            } else {
                setTraceError(
                    backendMsg || `Could not reach the backend to fetch this trace. The run may not be registered yet.`
                );
            }
    ...
    // -------------------------------------------------------------
    return (
        <div className="h-screen bg-[#030712] text-slate-100 flex flex-col font-sans overflow-hidden">
            {/* TOP HEADER */}
            <header className="border-b border-slate-800/80 bg-[#090d1a]/90 backdrop-blur-md sticky top-0 z-40 px-5 py-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${rightPanelOpen
            ...
            <div className="flex-1 flex overflow-hidden">
                ...
                <aside
                    className={`${leftPanelOpen ? "w-64 p-3" : "w-16 p-2"
                        } border-r border-slate-800/80 bg-[#070b16] flex flex-col justify-between shrink-0 select-none z-10 overflow-y-auto transition-all duration-200`}
                >
                    <div className="flex flex-col gap-1.5">
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl text-xs font-medium transition-all text-left ${leftPanelOpen ? "" : "justify-center px-0"
                                } ${activeTab === "overview"
                        ...
                        >
                            ...
                            {leftPanelOpen && "Unified Overview"}
                        </button>
                        ...
                        >
                            ...
                            {leftPanelOpen && "Trace & Log Explorer"}
                        </button>
                        ...
                        >
                            ...
                            {leftPanelOpen && "Root Cause Diffs"}
                        </button>

                        <button
                            onClick={() => setRightPanelOpen(true)}
                            title="AI Copilot & Insights"
                            className={`flex items-center px-3.5 py-2.5 rounded-xl text-xs font-medium transition-all text-left ${leftPanelOpen ? "justify-between" : "justify-center px-0"
                                } ${rightPanelOpen
                        ...
                        >
                            <span className="flex items-center gap-2.5">
                                ...
                                {leftPanelOpen && "AI Copilot & Insights"}
                            </span>
                            {leftPanelOpen && <Sparkles className="h-3 w-3 text-indigo-400" />}
                        </button>
                    </div>

                    {/* Bottom Left Profile Section — always reachable regardless of
                        page/section height, since the aside now has a bounded,
                        independently-scrollable height (see root layout fix). */}
                    <div className="pt-3 border-t border-slate-800/80 shrink-0">
                        <div
                            onClick={() => setShowProfileModal(true)}
                            title="Verification Engineer Profile"
                            className={`flex items-center p-2 rounded-xl hover:bg-slate-900 cursor-pointer border border-transparent hover:border-slate-800 transition-all ${leftPanelOpen ? "justify-between" : "justify-center"
                                }`}
                        >
                            ...
                            {leftPanelOpen && <User className="h-4 w-4 text-slate-400 shrink-0" />}
                        </div>
                    </div>
                </aside>

                {/* CENTER PANEL: DATA WORKSPACE */}
                <main className="flex-1 min-w-0 overflow-y-auto p-6 flex flex-col gap-6 transition-all duration-300">
                    {errorMsg && (
                        ...
                        <div className="flex flex-col gap-6 animate-fadeIn">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <div className="flex items-center bg-slate-900 border border-slate-800 p-1 rounded-lg text-xs flex-wrap gap-1">
                                        <button
                                            ...
                                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md font-medium transition-all ${overviewViewMode === "all"
                                        ...
                                        <button
                                            ...
                                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md font-medium transition-all ${overviewViewMode === "interactive"
                                                    ? "bg-indigo-600 text-white shadow-md shadow-indigo-600/30"
                                                    : "text-slate-400 hover:text-slate-200"
                                                }`}
                                        >
                                        ...
                                        <button
                                            ...
                                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md font-medium transition-all ${overviewViewMode === "xgboost_png"
                                                    ? "bg-indigo-600 text-white shadow-md shadow-indigo-600/30"
                                                    : "text-slate-400 hover:text-slate-200"
                                                }`}
                                        >
                                        ...
                                        <button
                                            ...
                                            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md font-medium transition-all ${overviewViewMode === "shap_png"
                                                    ? "bg-indigo-600 text-white shadow-md shadow-indigo-600/30"
                                                    : "text-slate-400 hover:text-slate-200"
                                                }`}
                                        >
                                                    ...
                                                    >
                                                        ...
                                                        <Bar dataKey="Importance_Score" radius={[0, 4, 4, 0]} isAnimationActive={false}>
                                                            {dashboardData.shap_metrics?.map((entry, index) => {
                                                                ...
                                                                return <Cell key={`cell-${index}`} fill={isRisk ? "#f87171" : "#10b981"} fillOpacity={0.85} />;
                                                            })}
                                                        </Bar>
                                                    </BarChart>
                                                </ResponsiveContainer>
                                            ...
                                            <div className="absolute inset-0">
                                                <ResponsiveContainer width="100%" height="100%">
                                                    <ScatterChart margin={{ top: 10, right: 30, left: 10, bottom: 20 }}>
                                                        <XAxis type="number" dataKey="Execution_Time_sec" name="Execution Time" unit="s" stroke="#64748b" fontSize={11} label={{ value: "Execution Time (s)", position: "insideBottom", offset: -10, fill: "#94a3b8", fontSize: 11 }} />
                                                        <YAxis type="number" dataKey="Peak_Memory_GB" name="Peak Memory" unit="GB" stroke="#64748b" fontSize={11} label={{ value: "Peak Memory (GB)", angle: -90, position: "insideLeft", fill: "#94a3b8", fontSize: 11 }} />
                                                        <ZAxis range={[30, 30]} />
                                                        <Tooltip cursor={{ strokeDasharray: "3 3", stroke: "#64748b" }} wrapperStyle={{ outline: "none", zIndex: 100 }} content={<CustomParetoTooltip />} />
                                                        {/* Disable animation to prevent 0-width mounting glitches */}
                                                        <Scatter name="Executions" data={dashboardData.pareto_front} isAnimationActive={false}>
                                                            {dashboardData.pareto_front?.map((entry, index) => (
                                                                <Cell key={`scatter-cell-${index}`} fill={entry.Is_Pareto ? "#10b981" : "#334155"} fillOpacity={entry.Is_Pareto ? 1 : 0.5} />
                                                            ))}
                                                        </Scatter>
                                                    </ScatterChart>
                                                </ResponsiveContainer>
                            ...
                            {overviewViewMode === "xgboost_png" && (
                                <div className="bg-[#0b1120] border border-slate-800 rounded-xl p-6 shadow-md flex flex-col gap-4 animate-fadeIn">
                                    ...
                                    <PlotArtifact
                                        sources={["/xgboost_importance.png", `${API_BASE}/api/plots/xgboost_importance.png`]}
                                        alt="Direct XGBoost Feature Importance Plot"
                                        label="XGBoost feature importance plot"
                                        heightClass="min-h-[360px] rounded-xl p-4"
                                        imgClass="max-h-[550px] w-auto rounded-lg shadow-xl object-contain border border-slate-800/80"
                                        onOpen={(src) =>
                                            setLightboxImage({
                                                src,
                                                title: "Direct XGBoost Feature Importance Plot",
                            ...
                            {overviewViewMode === "shap_png" && (
                                <div className="bg-[#0b1120] border border-slate-800 rounded-xl p-6 shadow-md flex flex-col gap-4 animate-fadeIn">
                                    ...
                                    <PlotArtifact
                                        sources={["/xgboost_shap_summary.png", `${API_BASE}/api/plots/xgboost_shap_summary.png`]}
                                        alt="Direct SHAP Beeswarm Summary Plot"
                                        label="SHAP beeswarm summary plot"
                                        heightClass="min-h-[360px] rounded-xl p-4"
                                        imgClass="max-h-[550px] w-auto rounded-lg shadow-xl object-contain border border-slate-800/80"
                                        onOpen={(src) =>
                                            setLightboxImage({
                                    ...
                                    <div className="flex items-center rounded-lg bg-slate-900 border border-slate-800 p-0.5 text-xs">
                                        {["ALL", "PASS", "FAIL"].map((st) => (
                                            <button
                                                ...
                                                className={`px-3 py-1 rounded-md font-medium transition-all ${runsStatusFilter === st
                                                        ? st === "FAIL"
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            <td className="py-2.5 px-3">
                                                                <span
                                                                    className={`px-2 py-0.5 rounded text-[10px] font-bold ${r.Status === "FAIL"
                                ...
                                <div className="lg:col-span-5 flex flex-col gap-4">
                                    <div className="bg-[#0b1120] border border-slate-800 rounded-xl p-4 shadow-md flex flex-col gap-3">
                                        ...
                                        {traceLoading ? (
                                            ...
                                            <div className="h-[420px] flex flex-col items-center justify-center text-center gap-3 px-6 border border-dashed border-slate-800 rounded-lg bg-black/30">
                                                <ShieldAlert className="h-6 w-6 text-amber-400" />
                                                <p className="text-xs text-slate-400 max-w-xs">{traceError}</p>
                                                <button
                                                    onClick={() => setShowIngestModal(true)}
                                                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-600/20 hover:bg-indigo-600/30 border border-indigo-500/30 text-indigo-300 text-[11px] font-medium transition-all"
                                                >
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td className="py-2.5 px-4">
                                                        <span
                                                            className={`px-2 py-0.5 rounded text-[10px] font-bold ${isMismatch
                ...
                {/* RIGHT PANEL: CONFIGINTEL AI COPILOT (fixed preset widths, no drag) */}
                {rightPanelOpen && (
                    <aside
                        style={{ width: rightPanelWidth }}
                        className="relative border-l border-slate-800/80 bg-[#080d1a] flex flex-col shrink-0 shadow-2xl animate-fadeIn select-none"
                    >
                        {/* Drawer Header */}
                        <div className="p-3.5 border-b border-slate-800 flex items-center justify-between bg-slate-900/60 select-auto">
                            <div className="flex items-center gap-2 min-w-0">
                                <Bot className="h-4 w-4 text-indigo-400 shrink-0" />
                                <span className="text-xs font-semibold text-white tracking-wide truncate">
                                    ConfigIntel AI Copilot
                                </span>
                                <span className="text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-1.5 py-0.2 rounded font-mono shrink-0">
                                    Gemini Free-Tier
                                </span>
                            </div>
                            <div className="flex items-center gap-1 shrink-0">
                                <button
                                    onClick={() =>
                                        setRightPanelWidthIdx((v) => (v + 1) % RIGHT_PANEL_WIDTHS.length)
                                    }
                                    className="text-slate-400 hover:text-white p-1 rounded hover:bg-slate-800"
                                    title={`Panel width: ${rightPanelWidth}px — click to cycle`}
                                >
                                    {rightPanelWidthIdx === RIGHT_PANEL_WIDTHS.length - 1 ? (
                        ...
                        {/* Diagnostic Body */}
                        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 text-xs select-auto">
                            ...
                            <div className="flex-1 flex flex-col gap-3 min-h-[180px]">
                                {copilotMessages.map((m, idx) => (
                                    <div
                                        key={idx}
                                        className={`p-3 rounded-xl leading-relaxed text-xs ${m.role === "user"
                                ...
                                {copilotLoading && (
                                    <div className="text-slate-400 text-xs italic flex items-center gap-2">
                                        ...
                                        ConfigIntel agent synthesizing logs...
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        {/* Input Bar */}
                        <div className="p-3 border-t border-slate-800 bg-slate-900/60 flex flex-col gap-2 select-auto">
